...
# IOU_THRESHOLD
# 物体検出（推論結果の後処理）において、**NMS（Non-Maximum Suppression, 非最大抑制）を適用する際の「バウンディングボックス間の重なり（IoU）」の上限値**。
# NMSでは、同じ物体に対して複数の検出ボックスが出力されることがあるため、「IoU（重なり率）」が`IOU_THRESHOLD`より大きいボックスどうしは**重複検出とみなし、信頼度の高い方以外を除外**します。
# この値の設定により、最終的に残る検出ボックスの数や重複具合が変わる。
# - **値を大きくすると**  
#   よほど大きく重なっている場合だけ除外されるため、重複ボックスが「残りやすく」なる（検出数が増えやすい）。
# - **値を小さくすると**  
#   少しの重なりでも除外されるため、重複ボックスは「減りやすく」なる（検出数は絞られやすい）。
# 推奨値は0.3～0.6程度、対象や用途・カメラの精度に応じて要調整。
# ----------------------------------------------
